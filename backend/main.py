...
def save_model(model, sequence_length, activation_function):
    model_dir = "models"
    os.makedirs(model_dir, exist_ok=True)
    model_path = os.path.join(model_dir, f"model_seq{sequence_length}_{activation_function}.pt")
...
async def train_model(
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            return JSONResponse({"error": f"Dataset must contain columns: {required_columns}"})

        data = df[["Open", "Volume", "Close"]].dropna()
        scaler = StandardScaler()
        data_scaled = scaler.fit_transform(data.values)

        train_data, test_data = train_test_split(data_scaled, test_size=(1 - train_split), shuffle=False)
        ...
        for activation_function in ["ELU", "Tanh"]:
            set_seed()
            model = CustomLSTM(input_size, hidden_size, output_size, getattr(torch.nn, activation_function)()).to(device)
            ...
            save_model(model, sequence_length, activation_function)

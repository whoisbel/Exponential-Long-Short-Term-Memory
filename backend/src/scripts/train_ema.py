...
def evaluate_model(model, X_data, y_true_scaled, model_name="Model"):
    ...
    preds_rescaled = close_scaler.inverse_transform(preds.reshape(-1, 1)).flatten()
    y_true_rescaled = close_scaler.inverse_transform(
...
HIDDEN_SIZES = 50
INPUT_SIZE = 4  # Changed to 4 (Close, EMA20, EMA50, Volume)
OUTPUT_SIZE = 1
NUM_LAYERS = 1
...
SAVE_DIR = "saved_models/test_ema/"
...
config_dict = {
    ...
    "features": {
        "inputs": ["Close", "EMA20", "EMA50", "Volume"],
        "target": "Close"
    },
...
# Calculate only EMA20 and EMA50
df['EMA20'] = df['Close'].ewm(span=20, adjust=False).mean()
df['EMA50'] = df['Close'].ewm(span=50, adjust=False).mean()

# Check for and handle NaN values
print(f"Shape before NaN handling: {df.shape}")
print(f"NaN values before handling:\n{df[['Close', 'EMA20', 'EMA50', 'Volume']].isna().sum()}")

# Drop rows with NaN values (these will be mostly at the beginning due to EMA calculation)
df = df.dropna(subset=['Close', 'EMA20', 'EMA50', 'Volume'])
print(f"Shape after NaN handling: {df.shape}")
print(f"NaN values after handling:\n{df[['Close', 'EMA20', 'EMA50', 'Volume']].isna().sum()}")

# Prepare features: Close, EMAs, and Volume
features = df[['Close', 'EMA20', 'EMA50', 'Volume']].values

# Create separate scalers for each feature
close_scaler = MinMaxScaler()
ema20_scaler = MinMaxScaler()
ema50_scaler = MinMaxScaler()
volume_scaler = MinMaxScaler()

# Scale each feature separately
scaled_close = close_scaler.fit_transform(features[:, 0].reshape(-1, 1))
scaled_ema20 = ema20_scaler.fit_transform(features[:, 1].reshape(-1, 1))
scaled_ema50 = ema50_scaler.fit_transform(features[:, 2].reshape(-1, 1))
scaled_volume = volume_scaler.fit_transform(features[:, 3].reshape(-1, 1))

# Combine scaled features
scaled_features = np.hstack((scaled_close, scaled_ema20, scaled_ema50, scaled_volume))

# For target variable scaling (only Close price)
scaler = close_scaler

X, y = [], []
for i in range(SEQ_LEN, len(scaled_features)):
    X.append(scaled_features[i - SEQ_LEN : i])
    y.append(scaled_close[i])  # We predict the Close price
...
def plot_predictions(model, X_data, title, color):
    ...
    with torch.no_grad():
        preds = model(X_data.to(DEVICE)).squeeze().cpu().numpy()
    preds = close_scaler.inverse_transform(preds.reshape(-1, 1))  # Use close_scaler for predictions
    return preds


actual = close_scaler.inverse_transform(y_test.squeeze().numpy().reshape(-1, 1))
actual_train = close_scaler.inverse_transform(y_train.squeeze().numpy().reshape(-1, 1))

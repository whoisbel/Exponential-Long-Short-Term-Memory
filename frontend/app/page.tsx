...
export default function Home() {
  ...
  const onSubmit = () => {
    ...
    Object.keys(modelConfigInput).forEach((key) => {
      formData.append(key, modelConfigInput[key]);
    });
    ...
    <div className="relative grid grid-rows-[auto_1fr_auto] items-center justify-items-center min-h-screen gap-6 font-[family-name:var(--font-geist-sans)]">
      {isLoading && (
        <div className="absolute inset-0 bg-black bg-opacity-50 z-50 flex flex-col items-center justify-center">
          ...
          <p className="mt-4 text-white text-lg font-semibold">Training the model...</p>
        </div>
      )}

      <nav className="flex justify-between items-center w-full h-[60px] shadow-lg bg-gray-800 text-white px-4">
        <p className="text-lg font-semibold">Thesis Prototype</p>
      </nav>
          ...
          {[
            ...
            <div key={key} className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={(e) =>
                  setModelConfigInput({ ...modelConfigInput, [key]: e.target.value })
                }
                className="border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            <table className="w-full border-collapse border border-gray-300">
              ...
              <tbody>
                {["rmse", "mae", "mse", "r2"].map((metric) => (
                  <tr key={metric}>
                    <td className="border border-gray-300 px-4 py-2 capitalize">{metric.toUpperCase()}</td>
                    <td className="border border-gray-300 px-4 py-2">{elu.metrics[metric].toFixed(4)}</td>
                    <td className="border border-gray-300 px-4 py-2">{tanh.metrics[metric].toFixed(4)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
